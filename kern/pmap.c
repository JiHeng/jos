...
mem_init(void)
{
	...
	boot_map_region(
		...
		KSTACKTOP - KSTKSIZE,
		KSTKSIZE,
		(physaddr_t)(PADDR(bootstacktop) - KSTKSIZE),
		PTE_W);
...
mem_init_mp(void)
{
	...
	for (int i = 0; i < NCPU; i++) {
		boot_map_region(
			...
			PADDR(&percpu_kstacks[i]), // percpu_kstacks is a piece of memory. we need to get the address of percpu_stacks[i]
			PTE_W);
	}
...
page_free(struct PageInfo *pp)
{
	...
	if (pp->pp_link != NULL || pp->pp_ref > 0) {
		panic("can not free in-use page\n");
	}
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if ((*pde & PTE_P) == 0) {
		// page for page table doesn't exist
		if (!create) {
			return NULL;
		}
...
check_kern_pgdir(void)
{
	...
	for (i = 0; i < npages * PGSIZE; i += PGSIZE) {
		if (check_va2pa(pgdir, KERNBASE + i) != i) {	
	        cprintf("%d: pa: %x, expected: %x\n", i, check_va2pa(pgdir, KERNBASE + i), i);
		}
...
check_page(void)
{
	...
	pte_t *p = KADDR(PTE_ADDR(*kern_pgdir));

...
bc_pgfault(struct UTrapframe *utf)
{
	...
	if ((r = sys_page_alloc(0, addr, PTE_SYSCALL)) < 0) {
		panic("in bc_pgfault, sys_page_map failed to alloc page: %e", r);
	}
...
flush_block(void *addr)
{
	...
	if ((uvpt[PGNUM(addr)] & (PTE_D | PTE_P)) == 0) {
		// cprintf("page not dirty or present, skip flush\n");
		return;
	}
	...
	int r;
	if((r = ide_write(sectno, addr, BLKSECTS)) < 0) {

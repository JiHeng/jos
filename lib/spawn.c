...
spawn(const char *prog, const char **argv)
{
	...
	child_tf.tf_eflags |= FL_IOPL_0;   // devious: see user/faultio.c
...
{
	for(char *addr = 0; (uint32_t)addr < ULIM; addr = addr + PGSIZE) {
		uint32_t idx = (uint32_t)addr / PGSIZE;
		if (uvpd[PDX(addr)] & (PTE_P | PTE_U)) {
			if (uvpt[idx] & PTE_SHARE) {
				sys_page_map(0, (void *)addr, child, (void *)addr, uvpt[idx] & PTE_SYSCALL);
			}
		}
	}
